...
let TiempoPirDebeEstarEncendido = 90000; // Tiempo de espera en la primera pantalla hasta que se muestra las imagenes y el video
...
function draw() {
  ...
  if (Etapa <= -1) {
    ...
    */
    ...
    // Dibujamos el gráfico sobre el canvas principal del calendario
    image(gfx, width * 0.56, 0, gfx.width * 0.66, gfx.height * 0.66);

    push();

    translate(width * 0.56 + 20, 20);
    ...
    translate(width * 0.56 + gfx.width * 0.66 - 20, 20);
    ...
    fill(255, 140);
    noStroke();
    rect(ArcX - 215, ArcY - 215, 430, 430, 200, 40, 200, 200);

    fill(174, 214, 241, 140);

    rect(width * 0.4, height * 0.4, width * 0.12, width * 0.12, 50, 50, 50, 50);
    ...
    stroke(255, 90, 150);
    arc(ArcX, ArcY, 400, 400, -125, TemperaturaDegrees - 125);
    stroke(150, 255, 90);
    arc(ArcX, ArcY, 360, 360, -125, HumidityDegrees - 125);
    stroke(150, 90, 255);
    arc(ArcX, ArcY, 320, 320, -125, PreasureDegrees - 125);

    push();
    imageMode(CENTER);
    image(ClimaIcon, width * 0.33, height * 0.425, 65, 65);

    // Mostrar la imagen del clima en el canvas
    if (weatherIcon) {
      image(weatherIcon, width * 0.461, height * 0.505, 200, 200);
    }
    ...
    image(
      ...
      width * 0.1,
      ArcY,
      IconTiempo.width * 0.6,
      IconTiempo.height * 0.6
    );
    ...
    strokeWeight(5);
    stroke(255, 90, 150);
    text(` ${temperature}°C`, width * 0.13, ArcY - 90);
    stroke(150, 255, 90);
    text(` ${humidity}%`, width * 0.13, ArcY - 28);
    let windSpeedKmH = windSpeed * 3.6; // Convertir m/s a km/h
    stroke(150, 90, 255);
    text(` ${windSpeedKmH.toFixed(2)} km/h`, width * 0.13, ArcY + 30); // Mostrar la velocidad en km/h
    stroke(253, 235, 208);
    text(` ${pressure} hPa`, width * 0.13, ArcY + 90);
    ...
    TemperaturaDegrees = map(temperatureNum, -15, 70, 10, 360);
    HumidityDegrees = map(humidityNum, 0, 100, 10, 360);
    PreasureDegrees = map(windSpeedKmH, 0, 80, 10, 360);
    ...
    strokeWeight(3);
    stroke(b, r, g);
    text(day() + " / " + month() + " / " + year(), width * 0.29, height * 0.29);
    ...
    things = font.textToPoints(hours + ":" + minutes + ":" + seconds, 400, 260);
...
let extensions = ['png', 'PNG', 'jpg', 'JPG', 'jpeg', 'webp', 'gif', 'avif'];

function loadImagesFromRepo(category, imgIndex, extIndex = 0) {
  if (Etapa <= -1) {
    // Si ya probamos todas las extensiones y ninguna funcionó, 
    // asumimos que no hay más imágenes en la secuencia.
    if (extIndex >= extensions.length) {
      console.log(`Finalizada la carga. Total imágenes en ${category}: ${maxImages}`);
      return;
    }

    let currentExt = extensions[extIndex];
    let url = `https://raw.githubusercontent.com/AdrianNF2005/San-Valero-Projectors/main/${category}/${imgIndex}.${currentExt}`;
    ...
    loadImage(
      ...
      (img) => {
        images.push(img);
        maxImages = imgIndex;
        // Imagen encontrada con éxito, pasamos a la SIGUIENTE imagen (empezando de nuevo por la primera extensión)
        loadImagesFromRepo(category, imgIndex + 1, 0);
      },
      (err) => {
        // Error: Probar la SIGUIENTE extensión para el MISMO índice de imagen
        loadImagesFromRepo(category, imgIndex, extIndex + 1);
      }
    );
  }
}
...
function InitialVideoYT() {
  ...
  // Cargar el video de YouTube usando la ID
  let videoURL = "https://youtu.be/eOoKMbxrkhE";
  let videoID = extractVideoID(videoURL);
...
function extractVideoID(url) {
  ...
  if (match && match[1]) {
    // Tomamos el resultado y lo "recortamos" a 11 caracteres por si acaso sobran
    let videoID = match[1].substring(0, 11);
    
    // Verificamos que ahora sí tenga exactamente 11
    if (videoID.length === 11) {
      return videoID;
    }
  }

  console.warn("URL de YouTube no válida:", cleanUrl);
  return "";
}
...
function cerrarIframe() {
  if (iframe) {
    ...
    if (typeof database !== 'undefined' && database !== null) {
      let ref = database.ref("Proyectores/Proyector" + ID_PROYECTOR + "JuegoSel");
      ref.set(-2);
    } else {
      console.warn("Database no definida aún. Reintentando...");
    }
  }
}
